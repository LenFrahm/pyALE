...
def analysis(
    ...
    for row_idx in range(meta_df.shape[0]):
        ...
        if meta_df.iloc[row_idx, 0] == "C":  # Contrast Analysis
            ...
            conditions = [
                meta_df.iloc[row_idx, 2:].dropna().to_list(),
                meta_df.iloc[row_idx + 1, 2:].dropna().to_list(),
            ]
            exp_idx1, masks, mask_names = compile_studies(conditions[0], tasks)
            exp_idxs = [exp_idx1, compile_studies(conditions[1], tasks)[0]]
            exp_dfs = [
                exp_all.loc[exp_idxs[0]].reset_index(drop=True),
            ...
            if len(exp_idxs[0]) >= 12 and len(exp_idxs[1]) >= 12:
                ...
                if not isfile(
                    f"Results/MainEffect/Full/Volumes/Corrected/{exp_names[1]}_cFWE05.nii"
                ):
                    main_effect(
                        exp_dfs[1],
                        exp_names[1],
        ...
        if meta_df.iloc[row_idx, 0][0] == "B":  # Balanced Contrast Analysis:
            ...
            if not isfile(
                f"Results/MainEffect/CV/Volumes/{exp_names[0]}_{target_n}.nii"
            ):
                main_effect(
                    exp_dfs[0],
                    exp_names[0],
            ...
            if not isfile(
                f"Results/MainEffect/CV/Volumes/{exp_names[1]}_{target_n}.nii"
            ):
                main_effect(
                    exp_dfs[1],
                    exp_names[1],
