...
def compute_ma(peaks, kernels):
    ...
    for i, kernel in enumerate(kernels):
        ma[i, :] = kernel_conv(peaks=peaks[i], kernel=kernel)
...
def compute_hx(ma, bin_edges):
    ...
    for i in range(ma.shape[0]):
        ...
        bin_idxs, counts = np.unique(
            np.digitize(data[prior], bin_edges), return_counts=True
        )
...
def compute_z(ale, hx_conv, step):
    ...
    z = norm.ppf(1 - p)
    z[z < 0] = 0
    z[small_p_mask] += ale[small_p_mask] * 2
    return z
...
def compute_null_ale(s0, sample_space, num_peaks, kernels):
    null_peaks = np.array(
        [
            sample_space[:, np.random.randint(0, sample_space.shape[1], num_peaks[i])].T
            for i in s0
        ],
        dtype=object,
    )
...
def compute_null_diff(s, prior, exp_dfs, target_n=None, diff_repeats=1000):
    ...
    for xi in (0, 1):
        null_peaks = np.array(
            [
                prior_idxs[
                    np.random.randint(0, prior_idxs.shape[0], exp_dfs[xi].Peaks[i]), :
                ]
                for i in s[xi]
            ],
            dtype=object,
    ...
    if target_n:
        p_diff = Parallel(n_jobs=4, verbose=1)(
            delayed(compute_ale_diff)(s, null_ma, prior, target_n)
            for i in range(diff_repeats)
        )
